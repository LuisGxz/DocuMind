...
import { AuthResult, UserDto } from './models';

const REFRESH_KEY = 'documind.refreshToken';
...
export class AuthService {
  ...
  async register(email: string, password: string, fullName: string): Promise<void> {
    const auth = await firstValueFrom(
      this.http.post<AuthResult>(`${API_BASE}/auth/register`, { email, password, fullName }),
    );
    this.storeSession(auth);
  }
  ...
  async tryRefresh(): Promise<boolean> {
    ...
    try {
      const auth = await firstValueFrom(
        this.http.post<AuthResult>(`${API_BASE}/auth/refresh`, { refreshToken }),
      );
      this.storeSession(auth);
      return true;
    } catch {
  ...
  async logout(): Promise<void> {
    ...
    if (refreshToken) {
      try {
        await firstValueFrom(this.http.post(`${API_BASE}/auth/logout`, { refreshToken }));
      } catch {
        /* server treats logout as idempotent */
      }
    }
    this.clearSession();
  }

  private storeSession(auth: AuthResult): void {
